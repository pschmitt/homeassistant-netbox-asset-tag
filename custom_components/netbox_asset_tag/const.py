...
SYNC_FIELD_SERIAL = "serial"
DEFAULT_SYNC_FIELDS: list[str] = [SYNC_FIELD_STATUS, SYNC_FIELD_LOCATION, SYNC_FIELD_NAME, SYNC_FIELD_HA_URL]
...
API_PAGE_SIZE = 250
# NetBox ignores the `asset_tag__isnull` lookup (it is not registered on the
# field) and silently returns every device, so the previous filter was a no-op.
# `asset_tag__empty=false` is the lookup NetBox actually honours. The interface
# and MAC-address endpoints have no usable server-side filter for "belongs to an
# asset-tagged device" (every traversal tested is silently ignored), and the
# inventory builder already skips untagged devices and tolerates interfaces/MACs
# that reference them, so those endpoints stay unfiltered.
API_DEVICES_PATH = "/api/dcim/devices/?asset_tag__empty=false"
